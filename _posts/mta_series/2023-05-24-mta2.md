...
description: >-
  This is my write-up for the Malware Traffic Analysis 2 challenge on the CyberDefenders platform
image: >-
...
We can check **Brim** for make sure that is the right IP is `172.16.165.132`:
...
To get the port number associated with this IP, just filter the data with the IP and you will get the port which is `51439`:
...
By Checking HTTP Conversations from statistics in **Wireshark**.. The conversation from the infected host `172.16.165.132` to that IP `192.30.138.146` has the most number of packets (1376):
...
checking the host name related to that ip, 
...
In Brim, just use `event_type="alert" Exploit Kit` to show alerts that contains "Exploit Kit" we will get the name of the Exploit Kit which is `Sweet Orange`:
...
To find the landing page of the exploit kit, I checked all http connections related to the exploit kit alert logs in the previous image in question7 and I found something intersting..
...
If you are using **Wireshark**, just go to Export Http Objects and search for application files that related to the the host which delivered the exploit-kit (we got it in question4 already)..

![](/assets/img/uploads/malware_traffic__analysis/mta2/16.png)

Save it and then upload the file on **VirusTotal** to know its MD5 hash.
...
answer will be `23/11/2014`

## Q15. When was the PE file compiled?

Back to **Question10**, check **details** page on **VirusTotal**, scroll down and check **Portable Executable Info** and you will see the **Compilation Timestamp** is `21/11/2014`:
...
Since the answer format needs only 2 methods and starts with D**,*** , then the right one will be `DEP,SEH`.